...
if (verifyEmailForm) {
 ...
 verifyEmailMsg.className = 'text-sm font-black p-2.5 rounded-xl border bg-rose-50 text-rose-800 border-rose-200 block';
 } finally {
...
if (sendOtpForm) sendOtpForm.addEventListener('submit', async (e) => {
 ...
 sendOtpMsg.className = 'text-sm font-black p-2.5 rounded-xl border bg-emerald-50 text-emerald-800 border-emerald-200 block';
 ...
 sendOtpMsg.className = 'text-sm font-black p-2.5 rounded-xl border bg-rose-50 text-rose-800 border-rose-200 block';
 } finally {
...
if (verifyOtpForm) verifyOtpForm.addEventListener('submit', async (e) => {
 ...
 verifyOtpMsg.className = 'text-sm font-black p-2.5 rounded-xl border bg-rose-50 text-rose-800 border-rose-200 block';
 } finally {
...
async function loadSubscriptionHistory() {
 ...
 container.innerHTML = '<p class="text-sm text-slate-900/50 font-bold py-4 text-center">لا توجد اشتراكات سابقة مسجلة بعد.</p>';
 return;
 }
 ...
 <th class="py-3 px-4">الخطة</th>
 <th class="py-3 px-4">المبلغ</th>
 <th class="py-3 px-4">تاريخ البدء</th>
...
async function loadPendingRequests() {
 ...
 <td class="py-3 px-4 font-bold text-slate-900/80 dir-ltr">${escapeHtml(r.email || '')}</td>
 <td class="py-3 px-4">${planText}</td>
...
function renderAdminUsers(users) {
 ...
 ? '<span class="px-4 py-1 rounded-full bg-amber-100 text-amber-900 border border-amber-300 text-[11px] font-black"> سنوي</span>'
 : (u.subscription_plan === 'monthly'
 ? '<span class="px-4 py-1 rounded-full bg-emerald-100 text-emerald-900 border border-emerald-300 text-[11px] font-black">️ شهري</span>'
 : '<span class="px-4 py-1 rounded-full bg-sky-100 text-sky-900 border border-sky-300 text-[11px] font-black"> تجريبي (7 أيام)</span>');

 const statusBadge = isExpired
 ? '<span class="px-4 py-1 rounded-full bg-rose-100 text-rose-900 border border-rose-300 text-[11px] font-black"> منتهي (مشاهدة فقط)</span>'
 : (u.subscription_status === 'suspended'
 ? '<span class="px-4 py-1 rounded-full bg-slate-200 text-slate-900 border border-slate-400 text-[11px] font-black"> موقوف</span>'
 : `<span class="px-4 py-1 rounded-full bg-emerald-100 text-emerald-900 border border-emerald-300 text-[11px] font-black"> نشط (${u.days_left} يوم)</span>`);
...
function renderVariantsEditPanel(product) {
 ...
 if (!variants.length) {
 list.innerHTML = '<p class="text-sm text-slate-900/40">لا توجد مواصفات/مقاسات مسجلة بعد.</p>';
 return;
 }
 ...
 return `
 <div class="flex items-center justify-between bg-white rounded-xl border border-slate-200/15 p-2.5 text-sm shadow-xs">
 <div class="flex items-center gap-2">
 ${imgThumb}
 ${colorCircle}
 <div>
...
async function syncRealAdDataToCalculator() {
 ...
 syncStatus.className = 'text-sm text-amber-800 bg-amber-50 p-2.5 rounded-xl border border-amber-200 block font-bold';
 }
 return;
 }
 ...
 syncStatus.className = 'text-sm text-emerald-900 bg-emerald-50 p-2.5 rounded-xl border border-emerald-300 block font-bold';
 }
...
function renderSavedCalculations() {
 ...
 <button type="button" data-calc-id="${item.id}" onclick="window.loadSavedCalculation('${item.id}')" class="btn-load-calc text-blue-600 font-black px-4 py-1 rounded-lg bg-blue-600/10 hover:bg-blue-600/20 transition-all cursor-pointer inline-flex items-center gap-1 shadow-2xs">
